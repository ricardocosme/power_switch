...
The system starts in the off state consuming only the current that is drawn at the power-down sleep mode. Using an ATtiny85 this is for example ~0.5uA@5V~. When the power switch is pressed the MCU wakes up and turns on the system delivering power. If the switch is pressed again the system is halted and the MCU goes to sleep.
